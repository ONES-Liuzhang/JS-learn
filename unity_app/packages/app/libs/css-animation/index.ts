...
import { requestAnimationTimeout, cancelAnimationTimeout } from '../request-animation-timeout'
...
function getStyleProperty(node: any, name: any) {
...
cssAnimation.style = (node: any, style: any, callback: any) => {
  if (node.rcEndListener) {
    node.rcEndListener()
  }
...
cssAnimation.setTransition = (node: any, p: any, value: any) => {
  let property = p
  let v = value
  if (value === undefined) {
    v = property
