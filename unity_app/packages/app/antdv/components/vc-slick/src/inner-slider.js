...
export default {
  ...
  methods: {
    ...
    play() {
      ...
      } else if (canGoNext({ ...this.$props, ...this.$data })) {
        nextIndex = this.currentSlide + this.slidesToScroll
      } else {
        return false
      }
  ...
  render() {
    ...
    if (this.vertical) {
      verticalHeightStyle = { height: typeof this.listHeight === 'number' ? `${this.listHeight}px` : this.listHeight }
    }
    ...
    if (this.vertical === false) {
      if (this.centerMode === true) {
        centerPaddingStyle = { padding: '0px ' + this.centerPadding }
      }
    } else if (this.centerMode === true) {
      centerPaddingStyle = { padding: this.centerPadding + ' 0px' }
    }

    const listStyle = { ...verticalHeightStyle, ...centerPaddingStyle }
    const { touchMove } = this
    let listProps = {
      directives: [
        {
          name: 'ant-ref',
