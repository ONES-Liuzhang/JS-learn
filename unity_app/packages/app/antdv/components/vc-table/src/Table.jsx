...
export default {
  ...
  created() {
    ['rowClick', 'rowDoubleclick', 'rowContextmenu', 'rowMouseenter', 'rowMouseleave'].forEach((name) => {
      warningOnce(this.$listeners[name] === undefined, `${name} is deprecated, please use customRow instead.`)
    })
  ...
  methods: {
    ...
    syncFixedTableRowHeight() {
      ...
      const { prefixCls } = this
      const headRows = this.ref_headTable ? this.ref_headTable.querySelectorAll('thead') : this.ref_bodyTable.querySelectorAll('thead')
    ...
    handleWheel(event) {
      ...
      if (window.navigator.userAgent.match(/Trident\/7\./) && scroll.y) {
        ...
        const { target } = event
        const { ref_bodyTable: bodyTable, ref_fixedColumnsBodyLeft: fixedColumnsBodyLeft, ref_fixedColumnsBodyRight: fixedColumnsBodyRight } = this
        let scrollTop = 0
