...
export const UNSELECTABLE_ATTRIBUTE = {unselectable: 'unselectable'}
...
export function getFilterTree(h, treeNodes, searchValue, filterFunc, valueEntities) {
  ...
  function mapFilteredNodeToData(node) {
    ...
    children = ((typeof children === 'function' ? children() : children) || []).map(mapFilteredNodeToData).filter((n) => n)
...
export function formatInternalValue(value, props) {
  ...
  return valueList.map((val) => ({value: val}))
}
...
function processEntity(entity, wrapper) {
  const {value} = getPropsData(entity.node)
  entity.value = value
