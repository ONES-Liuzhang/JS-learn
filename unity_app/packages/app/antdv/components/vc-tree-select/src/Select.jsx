...
const Select = {
  ...
  methods: {
    getDerivedStateFromProps(nextProps, prevState) {
      ...
      if (valueRefresh) {
        ...
        if (treeCheckable && !treeCheckStrictly) {
          ...
          newState._valueList = checkedKeys.map(key => ({value: (newState._keyEntities || prevState._keyEntities).get(key).value,}));
        } else {
    ...
    onMultipleSelectorRemove(event, removeValue) {
      ...
      const deselectInfo = {node: triggerNode,};
    ...
    onTreeNodeCheck(_, nodeEventInfo) {
      ...
      if (searchValue) {
        ...
        if (isAdd) {
          ...
          keyList = conductCheck([getPropsData(nodeEventInfo.node).eventKey], false, keyEntities, {checkedKeys: oriKeyList,}).checkedKeys;
        }
    ...
    onSearchInputChange(event) {
      ...
      if (!this.isSearchValueControlled()) {
        isSet = this.setUncontrolledState({_searchValue: value,});
        this.setOpenState(true);
      }
  ...
  render() {
    ...
    const popupProps = mergeProps(passProps, {
      ...
      on: {treeExpanded: this.delayForcePopupAlign,},
    });
