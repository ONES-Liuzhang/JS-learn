...
const MultipleSelector = {
  ...
  methods: {
    ...
    _renderPlaceholder() {
      const { prefixCls, placeholder, searchPlaceholder, searchValue, selectorValueList } = this.$props
      ...
      return (
        <span style={{ display: hidden ? 'none' : 'block' }} onClick={this.onPlaceholderClick} class={`${prefixCls}-search__field__placeholder`}>
          {currentPlaceholder}
        </span>
      )
    },
    ...
    renderSelection() {
      const { selectorValueList, choiceTransitionName, prefixCls, labelInValue, maxTagCount } = this.$props
      const {
      ...
      if (maxTagCount >= 0 && maxTagCount < selectorValueList.length) {
        ...
        if (typeof maxTagPlaceholder === 'string') {
          ...
          content = maxTagPlaceholder(labelInValue ? restValueList : restValueList.map(({ value }) => value))
        }
