...
export default {
  ...
  methods: {
    ...
    getDialogElement() {
      const { closable, prefixCls, width, height, title, footer: tempFooter, bodyStyle, visible, bodyProps, forceRender } = this
      const dest = {}
      if (width !== undefined) {
      ...
      if (closable) {
        ...
        closer = (
          <button type="button" key="close" onClick={this.close || noop} aria-label="Close" class={`${prefixCls}-close`}>
      ...
      const dialogTransitionProps = getTransitionProps(transitionName, {afterLeave: this.onAnimateLeave})
      return (
    ...
    getMaskElement() {
      ...
      if (props.mask) {
        ...
        maskElement = <LazyRenderBox v-show={props.visible} style={this.getMaskStyle()} key="mask" class={`${props.prefixCls}-mask`} {...props.maskProps} />
        if (maskTransition) {
