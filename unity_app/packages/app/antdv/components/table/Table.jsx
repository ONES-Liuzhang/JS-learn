...
export default {
  ...
  inject: { configProvider: { default: () => ConfigConsumerProps } },
  // CheckboxPropsCache: {
  //   [key: string]: any;
  // };
  // store: Store;
    ...
    rowSelection: {
      handler(val, oldVal) {
        if (val && 'selectedRowKeys' in val) {
          this.store.setState({ selectedRowKeys: val.selectedRowKeys || [] })
  ...
  methods: {
    ...
    handleSelectRow(selectionKey, index, onSelectFunc) {
      ...
      const changeableRowKeys = data.filter((item, i) => !this.getCheckboxPropsByItem(item, i).props.disabled).map((item, i) => this.getRecordKey(item, i))
    ...
    handlePageChange(current, ...otherArguments) {
      ...
      const newState = { sPagination: pagination }
      // Controlled current prop will not respond user interaction
    ...
    renderSelectionBox(type) {
      return (_, record, index) => {
        ...
        const handleChange = (e) => {
          ...
          type === 'radio' ? this.handleRadioSelect(record, index, e) : this.handleSelect(record, index, e)
        }
    ...
    renderRowSelection(prefixCls, locale) {
      ...
      if (rowSelection) {
        ...
        const selectionColumnClass = classNames(`${prefixCls}-selection-column`, { [`${prefixCls}-selection-column-custom`]: rowSelection.selections })
      ...
      return treeMap(columns, (column, i) => {
        ...
        if (column.sorter) {
          ...
          const ascend = sortDirections.indexOf('ascend') !== -1 && (
            <Icon class={`${prefixCls}-column-sorter-up ${isAscend ? 'on' : 'off'}`} type="caret-up" theme="filled" key="caret-up" />
          )
          const descend = sortDirections.indexOf('descend') !== -1 && (
            <Icon class={`${prefixCls}-column-sorter-down ${isDescend ? 'on' : 'off'}`} type="caret-down" theme="filled" key="caret-down" />
          )
          sortButton = (
          ...
          customHeaderCell = (col) => {
            ...
            if (column.customHeaderCell) {
              colProps = { ...column.customHeaderCell(col) }
            }
    ...
    renderPagination(prefixCls, paginationPosition) {
      ...
      return total > 0 && (position === paginationPosition || position === 'both') ? <Pagination {...paginationProps} /> : null
    },
    ...
    prepareParamsArguments(state) {
      ...
      const extra = { currentDataSource: this.getLocalData(state) }
  ...
  render() {
    ...
    const { renderEmpty } = this.configProvider
    ...
    const paginationPatchClass = this.hasPagination() && data && data.length !== 0 ? `${prefixCls}-with-pagination` : `${prefixCls}-without-pagination`
    const spinProps = {
      ...loading,
