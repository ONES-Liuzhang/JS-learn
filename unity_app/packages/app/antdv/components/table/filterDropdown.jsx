...
export default {
  ...
  data() {
    const visible = 'filterDropdownVisible' in this.column ? this.column.filterDropdownVisible : false
    ...
    renderMenus(items) {
      return items.map((item) => {
        if (item.children && item.children.length > 0) {
          ...
          const containSelected = Object.keys(sKeyPathOfSelectedItem).some((key) => sKeyPathOfSelectedItem[key].indexOf(item.value) >= 0)
    ...
    renderFilterIcon() {
      ...
      return filterIcon ? (
        cloneElement(filterIcon, {
          attrs: { title: locale.filterTitle },
          on: { click: stopPropagation },
          class: classNames(`${prefixCls}-icon`, dropdownIconClass)
        })
      ) : (
        <Icon title={locale.filterTitle} type="filter" theme="filled" class={dropdownIconClass} onClick={stopPropagation} />
      )
    }
  },
  ...
  render() {
    ...
    const multiple = 'filterMultiple' in column ? column.filterMultiple : true
    const dropdownMenuClass = classNames({ [`${dropdownPrefixCls}-menu-without-submenu`]: !this.hasSubMenu() })
