...
const Calendar = {
  ...
  watch: {
    ...
    selectedValue(val) {
      this.setState({ sSelectedValue: val })
    }
  },
  mounted() {
    this.$nextTick(() => {
  ...
  methods: {
    ...
    onKeyDown(event) {
      ...
      case KeyCode.ENTER:
        if (!disabledDate || !disabledDate(value)) {
          this.onSelect(value, { source: 'keyboard' })
        }
        event.preventDefault()
        return 1
      default:
    ...
    onDateInputChange(value) {
      this.onSelect(value, { source: 'dateInput' })
    },
    onDateInputSelect(value) {
      this.onSelect(value, { source: 'dateInputSelect' })
    },
    ...
    onToday() {
      ...
      this.onSelect(now, { source: 'todayButton' })
    },
    getRootDOMNode() {
      return this.$el
    },
  ...
  render() {
    ...
    const disabledTimeConfig = showTimePicker && disabledTime && timePicker ? getTimeConfig(sSelectedValue, disabledTime) : null
