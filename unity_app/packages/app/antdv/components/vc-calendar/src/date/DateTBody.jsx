...
const DateTBody = {
  ...
  render() {
    ...
    const { contentRender, prefixCls, selectedValue, value, showWeekNumber, dateRender, disabledDate, hoverValue } = props
    ...
    for (iIndex = 0; iIndex < DateConstants.DATE_ROW_COUNT; iIndex++) {
      ...
      for (jIndex = 0; jIndex < DateConstants.DATE_COL_COUNT; jIndex++) {
        ...
        if (dateRender) {
          ...
          dateHtml = (
            <div key={getIdFromDate(current)} class={dateClass} aria-selected={selected} aria-disabled={disabled}>
              {content}
            </div>
          )
        }
