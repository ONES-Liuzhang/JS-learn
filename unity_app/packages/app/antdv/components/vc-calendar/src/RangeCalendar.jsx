...
function normalizeAnchor(props, init) {
  ...
  const normalizedValue = value ? getValueFromSelectedValue(value) : getValueFromSelectedValue(selectedValue)
...
const RangeCalendar = {
  ...
  methods: {
    ...
    isAllowedDateAndTime(selectedValue) {
      return (
        isAllowedDate(selectedValue[0], this.disabledDate, this.disabledStartTime) && isAllowedDate(selectedValue[1], this.disabledDate, this.disabledEndTime)
      )
    },
    ...
    fireSelectValueChange(selectedValue, direct, cause) {
      ...
      if (!hasProp(this, 'selectedValue')) {
        this.setState({ sSelectedValue: selectedValue })
      }
    },
  ...
  render() {
    ...
    const { prefixCls, dateInputPlaceholder, timePicker, showOk, locale, showClear, showToday, type, seperator } = props
    ...
    const isTodayInView =
      (startValue.year() === thisYear && startValue.month() === thisMonth) || (endValue.year() === thisYear && endValue.month() === thisMonth)
    const nextMonthOfStart = startValue.clone().add(1, 'months')
    const isClosestMonths = nextMonthOfStart.year() === endValue.year() && nextMonthOfStart.month() === endValue.month()
    ...
    if (showOkButton) {
      const okButtonProps = mergeProps(baseProps, {
        props: { okDisabled: !this.isAllowedDateAndTime(sSelectedValue) || !this.hasSelectedValue() || sHoverValue.length },
        on: { ok: this.onOk }
      })
