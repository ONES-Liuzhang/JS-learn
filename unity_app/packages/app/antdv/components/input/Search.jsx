...
export default {
  ...
  methods: {
    ...
    renderSuffix(prefixCls) {
      ...
      const node = <Icon class={`${prefixCls}-icon`} type="search" key="searchIcon" onClick={this.onSearch} />
    ...
    renderAddonAfter(prefixCls) {
      ...
      if (
        ...
        (enterButtonAsElement.componentOptions && enterButtonAsElement.componentOptions.Ctor.extendOptions.__ANT_BUTTON)
      ) {
        ...
        button = (
          <Button class={btnClassName} type="primary" size={size} disabled={disabled} key="enterButton" onClick={this.onSearch}>
  ...
  render() {
    const { prefixCls: customizePrefixCls, inputPrefixCls: customizeInputPrefixCls, size, ...others } = getOptionProps(this)
