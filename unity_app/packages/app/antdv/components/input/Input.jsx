...
export default {
  ...
  data() {
    ...
    return { stateValue: !hasProp(this, 'value') ? defaultValue : value }
  },
  watch: {
    value(val) {
      this.stateValue = val
  ...
  methods: {
    ...
    renderClearIcon(prefixCls) {
      ...
      return <Icon type="close-circle" theme="filled" onClick={this.handleReset} class={`${prefixCls}-clear-icon`} role="button" />
    },
    ...
    renderInput(prefixCls) {
      const otherProps = omit(this.$props, ['prefixCls', 'addonBefore', 'addonAfter', 'prefix', 'suffix', 'allowClear', 'value', 'defaultValue', 'lazy'])
      const { stateValue, getInputClassName, handleKeyDown, handleChange, $listeners } = this
      const inputProps = {
        directives: [{ name: 'ant-input' }],
        domProps: { value: fixControlledValue(stateValue) },
      ...
      //     arrowPointAtCenter
      //   > <input {...inputProps} /></a-tooltip>
      // }

      return this.renderLabeledIcon(prefixCls, this.renderValidateContent(this.$createElement, <input {...inputProps} />))
    },
  },
  render() {
