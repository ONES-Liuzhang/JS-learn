...
export default {
    ...
    renderIconNode() {
      ...
      if (progressDot) {
        if (typeof progressDot === 'function') {
          iconNode = <span class={`${prefixCls}-icon`}>{progressDot({ index: stepNumber - 1, status, title, description, prefixCls })}</span>
        } else {
  ...
  render() {
    const { prefixCls, itemWidth, status = 'wait', tailContent, adjustMarginRight } = getOptionProps(this)
