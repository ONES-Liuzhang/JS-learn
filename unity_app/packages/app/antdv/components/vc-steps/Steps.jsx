...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      if (!isFlexSupported()) {
        this.setState({ flexSupported: false })
      }
    })
  },
  updated() {
    ...
    calcStepOffsetWidth() {
      ...
      if (domNode.children.length > 0) {
        ...
        this.calcTimeout = setTimeout(() => {
          ...
          // Reduce shake bug
          if (this.lastStepOffsetWidth === lastStepOffsetWidth || Math.abs(this.lastStepOffsetWidth - lastStepOffsetWidth) <= 3) {
            return
          }
  ...
  render() {
    const { prefixCls, direction, labelPlacement, iconPrefix, status, size, current, $scopedSlots, initial, icons } = this
    let { progressDot } = this
