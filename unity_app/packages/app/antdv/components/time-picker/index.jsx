...
const TimePicker = {
  ...
  methods: {
    ...
    renderInputIcon(prefixCls) {
      ...
      const clockIcon = (suffixIcon &&
        (isValidElement(suffixIcon) ? (
          cloneElement(suffixIcon, {class: `${prefixCls}-clock-icon`})
        ) : (
    ...
    renderTimePicker(locale) {
      ...
      const pickerClassName = {[`${prefixCls}-${size}`]: !!size}
      const tempAddon = getComponentFromProp(this, 'addon', {}, false)
      const pickerAddon = (panel) => {
        return tempAddon ? <div class={`${prefixCls}-panel-addon`}>{typeof tempAddon === 'function' ? tempAddon(panel) : tempAddon}</div> : null
      }
  ...
  render() {
    return <LocaleReceiver componentName="TimePicker" defaultLocale={defaultLocale} scopedSlots={{ default: this.renderTimePicker }} />
  }
}
