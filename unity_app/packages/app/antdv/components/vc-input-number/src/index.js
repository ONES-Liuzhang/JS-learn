...
export default {
  ...
  methods: {
    ...
    setValue(v, callback) {
      ...
      if (!hasProp(this, 'value')) {
        ...
        this.setState({ inputValue: this.toPrecisionAsStep(this.sValue) }, callback)
      }
      if (changed) {
  ...
  render() {
    const { prefixCls, disabled, readOnly, useTouch, autoComplete, upHandler, downHandler } = this.$props
      ...
      <div {...contentProps}>
        <div class={`${prefixCls}-handler-wrap`}>
          <InputHandler {...upHandlerProps}>
            {upHandler || <span unselectable="unselectable" class={`${prefixCls}-handler-up-inner`} onClick={preventDefault} />}
          </InputHandler>
          <InputHandler {...downHandlerProps}>
            {downHandler || <span unselectable="unselectable" class={`${prefixCls}-handler-down-inner`} onClick={preventDefault} />}
          </InputHandler>
        </div>
        <div class={`${prefixCls}-input-wrap`} role="spinbutton" aria-valuemin={this.min} aria-valuemax={this.max} aria-valuenow={sValue}>
          <input
