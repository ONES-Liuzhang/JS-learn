  ...
  methods: {
    onItemChange(type, itemValue) {
      ...
      if (type === 'hour') {
        if (use12Hours) {
          if (isAM) {
            value.hour(Number(itemValue) % 12)
          } else {
            value.hour((Number(itemValue) % 12) + 12)
          }
        } else {
          value.hour(Number(itemValue))
        }
      } else if (type === 'minute') {
        value.minute(Number(itemValue))
      } else if (type === 'ampm') {
    ...
    getMinuteSelect(minute) {
      const { prefixCls, minuteOptions, disabledMinutes, defaultOpenValue, showMinute, value: propValue } = this
      if (!showMinute) {
        return null
      }
    ...
    getSecondSelect(second) {
      const { prefixCls, secondOptions, disabledSeconds, showSecond, defaultOpenValue, value: propValue } = this
      if (!showSecond) {
        return null
      }
      const value = propValue || defaultOpenValue
