...
export default {
  ...
  methods: {
    ...
    getFormat() {
      ...
      if (use12Hours) {
        const fmtString = [showHour ? 'h' : '', showMinute ? 'mm' : '', showSecond ? 'ss' : ''].filter((item) => !!item).join(':')
    ...
    renderClearButton() {
      ...
      return (
        <a role="button" class={`${prefixCls}-clear`} title={clearText} onClick={this.onClear} tabIndex={0}>
