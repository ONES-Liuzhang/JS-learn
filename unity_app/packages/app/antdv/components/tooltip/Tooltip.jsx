...
export default {
  ...
  methods: {
    ...
    getDisabledCompatibleChildren(ele) {
      ...
      if (
        (isAntBtn && (ele.componentOptions.propsData.disabled || ele.componentOptions.propsData.disabled === '')) ||
        // eslint-disable-next-line no-undefined
        (ele.tag === 'button' && ele.data && ele.data.attrs && ele.data.attrs.disabled !== undefined)
      ) {
        // Pick some layout related style properties up to span
        // Prevent layout bugs like https://github.com/ant-design/ant-design/issues/5254
        const { picked, omitted } = splitObject(getStyle(ele), ['position', 'left', 'right', 'top', 'bottom', 'float', 'display', 'zIndex'])
        const spanStyle = {
  ...
  render() {
    ...
    const childCls = { [openClassName || `${prefixCls}-open`]: true }
    const tooltipProps = {
      props: {
        ...$props,
        prefixCls,
