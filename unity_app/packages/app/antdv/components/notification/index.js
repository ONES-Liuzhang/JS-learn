...
function notice(args) {
  ...
  let iconNode = null
  if (icon) {
    iconNode = (h) => <span class={`${prefixCls}-icon`}> {typeof icon === 'function' ? icon(h) : icon} </span>
  } else if (type) {
    ...
    notification.notice({
      content: (h) => (
        <div class={iconNode ? `${prefixCls}-with-icon` : ''}>
          ...
          <div class={`${prefixCls}-description`}> {typeof description === 'function' ? description(h) : description} </div>{' '}
          {btn ? <span class={`${prefixCls}-btn`}> {typeof btn === 'function' ? btn(h) : btn} </span> : null}
        </div>
      ),
      duration,
      closable: true,
