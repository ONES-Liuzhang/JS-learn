...
const Transfer = {
  ...
  methods: {
    ...
    moveTo(direction) {
      ...
      this.setState({ [this.getSelectedKeysName(oppositeDirection)]: [] })
    ...
    handleSelectAll(direction, filteredDataSource, checkAll) {
      ...
      if (!this.selectedKeys) {
        this.setState({ [this.getSelectedKeysName(direction)]: holder })
      }
    },
    ...
    getLocale(transferLocale, renderEmpty) {
      ...
      const oldLocale = { notFoundContent: renderEmpty(h, 'Transfer') }
    ...
    renderTransfer(transferLocale) {
      ...
      const { prefixCls: customizePrefixCls, disabled, operations = [], showSearch, listStyle, operationStyle, filterOption, lazy } = props
  ...
  render() {
    return <LocaleReceiver componentName="Transfer" defaultLocale={defaultLocale.Transfer} scopedSlots={{ default: this.renderTransfer }} />
  },
}
