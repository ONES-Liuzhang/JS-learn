...
export default {
  ...
  render() {
    ...
    const listCls = classNames(prefixCls, {[`${prefixCls}-with-footer`]: !!footerDom})
    ...
    const transitionProps = getTransitionProps(transitionName, {leave: noop})

    const searchNotFound = showItems.every((item) => item === null) && <div class={`${prefixCls}-body-not-found`}>{notFoundContent}</div>
    const listBody = bodyDom || (
      <div class={classNames(showSearch ? `${prefixCls}-body ${prefixCls}-body-with-search` : `${prefixCls}-body`)}>
        {search}
        {!searchNotFound && (
          <transition-group {...transitionProps} tag="ul" class={`${prefixCls}-content`} ref="listContentWrapper">
            {showItems}
          </transition-group>
        )}
        {searchNotFound}
