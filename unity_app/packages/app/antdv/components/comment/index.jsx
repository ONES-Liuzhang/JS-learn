...
const Comment = {
  ...
  render() {
    ...
    const avatarDom = <div class={`${prefixCls}-avatar`}>{typeof avatar === 'string' ? <img src={avatar} /> : avatar}</div>

    const actionDom = actions && actions.length ? <ul class={`${prefixCls}-actions`}>{this.getAction(actions)}</ul> : null
