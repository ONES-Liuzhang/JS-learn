...
const Select = {
  ...
  methods: {
    getDerivedStateFromProps(nextProps, prevState) {
      const optionsInfo = prevState._skipBuildOptionsInfo ? prevState._optionsInfo : this.getOptionsInfoFromProps(nextProps, prevState)
    ...
    onInputChange(e) {
      ...
      if (composing || _inputValue === val) {
        this.setState({ _mirrorInputValue: val })
        return
      }
    ...
    inputBlur(e) {
      ...
      if (
        ...
        (e.relatedTarget === this.$refs.arrow ||
          (target && this.selectTriggerRef && this.selectTriggerRef.getInnerMenu() && this.selectTriggerRef.getInnerMenu().$el === target) ||
    ...
    _getInputElement() {
      ...
      const inputCls = classnames(getClass(inputElement), { [`${props.prefixCls}-search__field`]: true })
    ...
    setInputValue(inputValue, fireSearch = true) {
      if (inputValue !== this.$data._inputValue) {
        this.setState({ _inputValue: inputValue }, this.forcePopupAlign)
        if (fireSearch) {
    ...
    fireChange(value) {
      if (!hasProp(this, 'value')) {
        this.setState({ _value: value }, this.forcePopupAlign)
      }
      ...
      children.forEach((child) => {
        ...
        if (getSlotOptions(child).isSelectOptGroup) {
          ...
          if (inputValue && this._filterOption(inputValue, child)) {
            ...
            const innerItems = this.renderFilterOptionsFromChildren(childChildren, childrenKeys, menuItems)
        ...
        warning(
          ...
          'the children of `Select` should be `Select.Option` or `Select.OptGroup`, ' + `instead of \`${getSlotOptions(child).name || getSlotOptions(child)}\`.`
        )
    ...
    renderTopControlNode() {
      ...
      if (isSingleMode(props)) {
        ...
        if (!showSearch) {
          ...
          innerNode = [
            selectedValue,
            <div class={`${prefixCls}-search ${prefixCls}-search--inline`} key="input" style={{ display: open ? 'block' : 'none' }}>
        ...
        if (isMultipleOrTags(props)) {
          selectedValueNodes = limitedCountValue.map((singleValue) => {
            ...
            const choiceClassName = disabled ? `${prefixCls}-selection__choice ${prefixCls}-selection__choice__disabled` : `${prefixCls}-selection__choice`
        ...
        if (isMultipleOrTags(props) && choiceTransitionName) {
          ...
          innerNode = <transition-group {...transitionProps}>{selectedValueNodes}</transition-group>
        } else {
    ...
    renderArrow(multiple) {
      ...
      const defaultIcon = loading ? <i class={`${prefixCls}-arrow-loading`} /> : <i class={`${prefixCls}-arrow-icon`} />
      return (
        <span
          key="arrow"
