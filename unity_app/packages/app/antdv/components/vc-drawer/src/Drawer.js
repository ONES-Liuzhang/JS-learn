...
const Drawer = {
  ...
  methods: {
    ...
    getParentAndLevelDom(props) {
      ...
      if (level === 'all') {
        ...
        children.forEach((child) => {
          if (child.nodeName !== 'SCRIPT' && child.nodeName !== 'STYLE' && child.nodeName !== 'LINK' && child !== this.container) {
    ...
    setLevelDomTransform(open, openTransition, placementName, value) {
      ...
      if (!windowIsUndefined) {
        ...
        if (getContainer === 'body') {
          ...
          const right =
            document.body.scrollHeight > (window.innerHeight || document.documentElement.clientHeight) && window.innerWidth > document.body.offsetWidth ?
              getScrollBarSize(1) :
              0
          ...
          if (open && document.body.style.overflow !== 'hidden') {
            ...
            domArray.forEach((item, i) => {
              ...
              addEventListener(item, eventArray[i] || 'touchmove', i ? this.removeMoveHandler : this.removeStartHandler, this.passive)
            })
            ...
            if ((this.isOpenChange || openTransition) && right) {
              ...
              clearTimeout(this.timeout)
              this.timeout = setTimeout(() => {
                this.dom.style.transition = `${trannsformTransition},${heightTransition ? `${heightTransition},` : ''}${widthTransition}`
                this.dom.style.transform = ''
                this.dom.style.msTransform = ''
                this.dom.style.width = ''
            ...
            domArray.forEach((item, i) => {
              ...
              removeEventListener(item, eventArray[i] || 'touchmove', i ? this.removeMoveHandler : this.removeStartHandler, this.passive)
            })
          }
        }
      }
    ...
    getChildToRender(open) {
      const { className, prefixCls, placement, handler, showMask, maskStyle, width, height, wrapStyle } = this.$props
      ...
      if (isOpenChange === undefined || isOpenChange) {
        const contentValue = this.contentDom ? this.contentDom.getBoundingClientRect()[isHorizontal ? 'width' : 'height'] : 0
      ...
      const domContProps = {
        ...
        on: { transitionend: this.onWrapperTransitionEnd },
        style: wrapStyle
      }
      ...
      return (
        <div {...domContProps}>
          {showMask && <div class={`${prefixCls}-mask`} onClick={this.onMaskTouchEnd} style={maskStyle} {...{ directives: directivesMaskDom }} />}
          <div
    ...
    getTouchParentScroll(root, currentTarget, differX, differY) {
      ...
      if (
        (isY &&
          (!scrollY ||
            !(currentT - t) ||
            (scrollY && ((currentTarget.scrollTop >= scrollY && differY < 0) || (currentTarget.scrollTop <= 0 && differY > 0))))) ||
        (isX &&
          (!scrollX ||
            !(currentL - l) ||
            (scrollX && ((currentTarget.scrollLeft >= scrollX && differX < 0) || (currentTarget.scrollLeft <= 0 && differX > 0)))))
      ) {
