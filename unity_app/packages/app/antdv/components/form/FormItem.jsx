...
function comeFromSlot(vnodes = [], itemVnode) {
  ...
  for (let i = 0, len = vnodes.length; i < len; i++) {
    ...
    if (vnode && (vnode === itemVnode || vnode.$vnode === itemVnode)) {
      ...
      const componentOptions = vnode.componentOptions || (vnode.$vnode && vnode.$vnode.componentOptions)
...
export default {
  ...
  mounted() {
    ...
    warning(
      this.getControls(this.slotDefault, true).length <= 1 || help !== undefined || validateStatus !== undefined,
  ...
  methods: {
    ...
    renderValidateWrapper(prefixCls, defaultVnode, extraVnode) {
      const props = this.$props
      const onlyControl = this.getOnlyControl
      const validateStatus = props.validateStatus === undefined && onlyControl ? this.getValidateStatus() : props.validateStatus
      ...
      // const help = this.getHelpMessage()
      // if (help) {
      //   return <a-tooltip placement="topLeft" getPopupContainer={this.getPopupContainer}>
    ...
    renderLabel(prefixCls) {
      ...
      const labelClassName = classNames({ [`${prefixCls}-item-required`]: required })
      ...
      return label ? (
        <Col {...colProps}>
          <label for={id || this.getId()} class={labelClassName} title={typeof label === 'string' ? label : ''} onClick={this.onLabelClick}>
            {labelChildren}
          </label>
        </Col>
      ) : null
    ...
    renderChildren(prefixCls) {
      const labelNode = this.renderLabel(prefixCls)
      const validateWrapperNode = this.renderValidateWrapper(prefixCls,
        this.slotDefault,
        this.renderExtra(prefixCls),
      )
      if (!labelNode) {
        return validateWrapperNode
      }
      return [
        labelNode,
        this.renderWrapper(prefixCls, validateWrapperNode),
      ]
    },
    renderFormItem() {
  ...
  render() {
    ...
    if (decoratorFormProps.form && fieldDecoratorId && child.length) {
      const { getFieldDecorator } = decoratorFormProps.form
