...
export default {
  ...
  watch: {
    __propsSymbol__() {
      ...
      if ('activeKey' in nextProps) {
        this.setState({ _activeKey: nextProps.activeKey })
      } else if (!activeKeyIsValid(nextProps, this.$data._activeKey)) {
        // https://github.com/ant-design/ant-design/issues/7093
        this.setState({ _activeKey: getDefaultActiveKey(nextProps) })
      }
    },
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    setActiveKey(activeKey) {
      if (this.$data._activeKey !== activeKey) {
        const props = getOptionProps(this)
        if (!('activeKey' in props)) {
          this.setState({ _activeKey: activeKey })
        }
  ...
  render() {
    ...
    const { prefixCls, navWrapper, tabBarPosition, renderTabContent, renderTabBar, destroyInactiveTabPane } = props
    const cls = {
      [prefixCls]: 1,
    ...
    const sentinelStart = <Sentinel key="sentinelStart" setRef={this.setSentinelStart} nextElement={this.panelSentinelStart} />
    const sentinelEnd = <Sentinel key="sentinelEnd" setRef={this.setSentinelEnd} prevElement={this.panelSentinelEnd} />
