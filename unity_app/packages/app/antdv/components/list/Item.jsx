...
export default {
  ...
  render() {
    ...
    const contentClassString = classNames(`${prefixCls}-item-content`, {[`${prefixCls}-item-content-single`]: metaContent.length < 1})
    const content = otherContent.length > 0 ? <div class={contentClassString}>{otherContent}</div> : null
    ...
    if (actions && actions.length > 0) {
      ...
      actionsContent = <ul class={`${prefixCls}-item-action`}>{actions.map((action, i) => actionsContentItem(action, i))}</ul>
    }
