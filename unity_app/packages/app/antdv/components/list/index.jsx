...
const List = {
  ...
  methods: {
    ...
    renderEmpty(prefixCls, renderEmpty) {
      ...
      const locale = this
      return <div class={`${prefixCls}-empty-text`}>{(locale && locale.emptyText) || renderEmpty(h, 'List')}</div>
    }
  },
  ...
  render() {
    ...
    if (typeof loadingProp === 'boolean') {
      loadingProp = { spinning: loadingProp }
    }
    ...
    if (splitDataSource.length > 0) {
      ...
      const childrenList = items.map((child, index) => cloneElement(child, { key: this.keys[index] }))
