...
export default {
  ...
  methods: {
    ...
    onSelect(keys, event) {
      ...
      if (multiple && ctrlPick) {
        ...
        // Shift click
        newSelectedKeys = Array.from(new Set([...(this.cachedSelectedKeys || []), ...calcRangeKeys(children, expandedKeys, eventKey, this.lastSelectedKey)]))
      } else {
        // Single click
  ...
  render() {
    ...
    warning(!this.$listeners.doubleclick, '`doubleclick` is deprecated. please use `dblclick` instead.')
    const treeProps = {
      props: {
        icon: getIcon,
        ...props,
