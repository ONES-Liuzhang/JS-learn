...
export default {
  ...
  methods: {
    renderProcessInfo(prefixCls, progressStatus) {
      ...
      let text
      const textFormatter = format || this.$scopedSlots.format || ((percentNumber) => `${percentNumber}%`)
  ...
  render() {
    ...
    const progressStatus =
      parseInt(successPercent !== undefined ? successPercent.toString() : percent.toString(), 10) >= 100 && !('status' in props) ?
        'success' :
        status || 'normal'
    let progress
