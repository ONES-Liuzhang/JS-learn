...
import { fieldMixin } from '../_util/ruleMixin'
...
const Value = PropTypes.shape({ key: PropTypes.oneOfType([PropTypes.string, PropTypes.number]) }).loose
...
const Select = {
  ...
  methods: {
    ...
    renderSuffixIcon(prefixCls) {
      ...
      return <Icon type="caret-down" class={`${prefixCls}-arrow-icon`} />
    },
  },
  render(h) {
    const { prefixCls: customizePrefixCls, size, mode, options, getPopupContainer, ...restProps } = getOptionProps(this)
    ...
    const finalRemoveIcon = (removeIcon && (isValidElement(removeIcon) ? cloneElement(removeIcon, { class: `${prefixCls}-remove-icon` }) : removeIcon)) || (
      <Icon type="close" class={`${prefixCls}-remove-icon`} />
    )

    const finalClearIcon = (clearIcon && (isValidElement(clearIcon) ? cloneElement(clearIcon, { class: `${prefixCls}-clear-icon` }) : clearIcon)) || (
    ...
    const finalMenuItemSelectedIcon = (menuItemSelectedIcon &&
      ...
      (
        <Icon type="success" class={`${prefixCls}-selected-icon`} />
      )
    ...
    const selectProps = {
      ...
      on: { ...this.$listeners, ...this.ruleListeners },
      class: cls,
      ref: 'vcSelect',
    }
    return this.renderValidateContent(h, <VcSelect {...selectProps} />)
  },
}
