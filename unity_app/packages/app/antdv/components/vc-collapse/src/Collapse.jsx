...
export default {
  ...
  watch: {
    activeKey(val) {
      this.setState({ stateActiveKey: _toArray(val) })
    },
    openAnimation(val) {
      this.setState({ currentOpenAnimations: val })
    }
  },
  methods: {
    onClickItem(key) {
    ...
    getNewChild(child, index) {
      ...
      if (!disabled && disabled !== '') {
        panelEvents = { itemClick: this.onClickItem }
      }
  ...
  render() {
    ...
    const collapseClassName = { [prefixCls]: true }
    return (
