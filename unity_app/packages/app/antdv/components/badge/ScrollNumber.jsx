...
export default {
  ...
  watch: {
    count(val) {
      if (this.sCount !== val) {
        ...
        // 复原数字初始位置
        this.setState({ animateStarted: true }, () => {
          // 等待数字位置复原完毕
          // 开始设置完整的数字
          setTimeout(() => {
            this.setState({ animateStarted: false, sCount: val }, () => {
              this.$emit('animated')
            })
          }, 5)
        })
  ...
  render() {
    const { prefixCls: customizePrefixCls, title, component: Tag = 'sup', displayComponent, className } = this
    const { getPrefixCls } = this.configProvider
    const prefixCls = getPrefixCls('scroll-number', customizePrefixCls)
    if (displayComponent) {
      return cloneElement(displayComponent, { class: `${prefixCls}-custom-component` })
    }
