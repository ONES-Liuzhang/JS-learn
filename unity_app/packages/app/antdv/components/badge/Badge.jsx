...
export default {
  ...
  methods: {
    ...
    renderDispayComponent() {
      ...
      return cloneElement(customNode, { style: this.getStyleWithOffset() })
    },
  ...
  render() {
    const { prefixCls: customizePrefixCls, scrollNumberPrefixCls: customizeScrollNumberPrefixCls, status, text, $slots } = this
    ...
    if (!children.length && status) {
      return (
        <span {...{ on: this.$listeners }} class={this.getBadgeClassName(prefixCls)} style={this.getStyleWithOffset()}>
