...
export default {
  name: 'ATimeline',
  props: initDefaultProps(TimelineProps, { reverse: false }),
  ...
  render() {
    ...
    const timeLineItems = reverse ? [pendingItem, ...children.reverse()] : [...children, pendingItem]
    ...
    const items = truthyItems.map((ele, idx) =>
      cloneElement(ele, {
        class: classNames([
          !reverse && !!pending ? (idx === itemsCount - 2 ? lastCls : '') : idx === itemsCount - 1 ? lastCls : '',
          mode === 'alternate' ? (idx % 2 === 0 ? `${prefixCls}-item-left` : `${prefixCls}-item-right`) : mode === 'right' ? `${prefixCls}-item-right` : ''
        ])
      })
    )
