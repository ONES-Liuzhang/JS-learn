...
const Tree = {
  ...
  methods: {
    getDerivedStateFromProps(props, prevState) {
      ...
      const newState = { _prevProps: { ...props } }
      const self = this
      ...
      if (needSync('expandedKeys') || (_prevProps && needSync('autoExpandParent'))) {
        newState._expandedKeys =
          props.autoExpandParent || (!_prevProps && props.defaultExpandParent) ? conductExpandParent(props.expandedKeys, keyEntities) : props.expandedKeys
      ...
      setTimeout(() => {
        ...
        this.delayedDragEnterLogic[pos] = setTimeout(() => {
          ...
          if (!hasProp(this, 'expandedKeys')) {
            this.setState({ _expandedKeys: newExpandedKeys })
          }
    ...
    onNodeDragOver(event, node) {
      ...
      if (this.dragNode && eventKey === _dragOverNodeKey && node.$refs.selectHandle) {
        ...
        this.setState({ _dropPosition: dropPosition })
      }
      this.__emit('dragover', { event, node })
    },
    onNodeDragLeave(event, node) {
      this.setState({ _dragOverNodeKey: '' })
      ...
      return new Promise((resolve) => {
        ...
        this.setState(({ _loadedKeys: loadedKeys = [], _loadingKeys: loadingKeys = [] }) => {
          ...
          promise.then(() => {
            ...
            this.setUncontrolledState({ _loadedKeys: newLoadedKeys })
            this.setState({ _loadingKeys: newLoadingKeys })
            resolve()
          })

          return { _loadingKeys: arrAdd(loadingKeys, eventKey) }
        })
      })
    },
  ...
  render() {
    ...
    return (
      <ul
        {...domProps}
        class={classNames(prefixCls, { [`${prefixCls}-show-line`]: showLine })}
        role="tree"
        unselectable="on"
