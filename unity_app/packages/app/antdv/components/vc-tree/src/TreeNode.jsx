...
const TreeNode = {
  ...
  methods: {
    ...
    onCheck(e) {
      ...
      const { vcTree: { checkable, onNodeCheck } } = this
    ...
    onDragStart(e) {
      ...
      e.stopPropagation()
      this.setState({ dragNodeHighlight: true })
    ...
    onDragEnd(e) {
      ...
      e.stopPropagation()
      this.setState({ dragNodeHighlight: false })
    ...
    renderSwitcher() {
      ...
      if (this.isLeaf2()) {
        return (
          <span key="switcher" class={classNames(`${prefixCls}-switcher`, `${prefixCls}-switcher-noop`)}>
            {typeof switcherIcon === 'function' ? cloneElement(switcherIcon({ ...this.$props, isLeaf: true })) : switcherIcon}
          </span>
        )
      }
      ...
      return (
        <span key="switcher" onClick={this.onExpand} class={switcherCls}>
          {typeof switcherIcon === 'function' ? cloneElement(switcherIcon({ ...this.$props, isLeaf: false })) : switcherIcon}
        </span>
      )
    },
    ...
    renderChildren() {
      ...
      const { vcTree: { prefixCls, openTransitionName, openAnimation, renderTreeNode } } = this
      ...
      if (expanded) {
        $children = (
          <ul class={classNames(`${prefixCls}-child-tree`, expanded && `${prefixCls}-child-tree-open`)} data-expanded={expanded} role="group">
  ...
  render(h) {
    const { dragOver, dragOverGapTop, dragOverGapBottom, isLeaf, expanded, selected, checked, halfChecked, loading } = this.$props
    const { vcTree: { prefixCls, filterTreeNode, draggable } } = this
    const disabled = this.isDisabled()
    return (
      <li
        class={{
