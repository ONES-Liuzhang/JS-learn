...
const DOMWrap = {
  ...
  methods: {
    ...
    setChildrenWidthAndResize() {
      ...
      const overflowedItems = menuItemNodes.filter((c) => c.className.split(' ').indexOf(MENUITEM_OVERFLOWED_CLASSNAME) >= 0)
    ...
    renderChildren(children) {
      ...
      return (children || []).reduce((acc, childNode, index) => {
        let item = childNode
        const { eventKey } = getPropsData(childNode)
        if (this.mode === 'horizontal') {
  ...
  render() {
    ...
    const tagProps = { on: this.$listeners }
