...
export default {
  ...
  methods: {
    ...
    handleKeyUp(e) {
      ...
      const { stateCurrentInputValue } = this

      if (value !== stateCurrentInputValue) {
        this.setState({ stateCurrentInputValue: value })
      }
    ...
    changePageSize(size) {
      ...
      if (typeof size === 'number') {
        if (!hasProp(this, 'pageSize')) {
          this.setState({ statePageSize: size })
        }
        if (!hasProp(this, 'current')) {
          this.setState({
            stateCurrent: current,
  ...
  render() {
    ...
    if (this.simple) {
      if (goButton) {
        ...
        gotoButton = (
          <li title={this.showTitle ? `${locale.jump_to}${this.stateCurrent}/${allPages}` : null} class={`${prefixCls}-simple-pager`}>
            {gotoButton}
          </li>
        )
      }
      ...
      return (
        <ul class={`${prefixCls} ${prefixCls}-simple`}>
          ...
          <li title={this.showTitle ? `${stateCurrent}/${allPages}` : null} class={`${prefixCls}-simple-pager`}>
            <input
              type="text"
    ...
    if (allPages <= 5 + pageBufferSize * 2) {
      ...
      if (!allPages) {
        pagerList.push(<Pager {...pagerProps} key="noPager" page={allPages} class={`${prefixCls}-disabled`} />)
      }
    ...
    return (
      <ul class={{ [`${prefixCls}`]: true, [`${prefixCls}-disabled`]: disabled }} unselectable="unselectable" ref="paginationNode">
        {totalText}
        <li
          title={this.showTitle ? locale.prev_page : null}
          onClick={this.prev}
