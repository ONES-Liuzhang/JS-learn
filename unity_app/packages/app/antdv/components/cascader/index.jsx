...
const Cascader = {
  ...
  methods: {
    ...
    highlightKeyword(str, keyword, prefixCls) {
      return str.split(keyword).map((node, index) => (index === 0 ? node : [<span class={`${prefixCls}-menu-item-keyword`}>{keyword}</span>, node]))
    },

    defaultRenderFilteredOption({ inputValue, path, prefixCls, names }) {
      return path.map((option, index) => {
        const label = option[names.label]
        const node = label.indexOf(inputValue) > -1 ? this.highlightKeyword(label, inputValue, prefixCls) : label
  ...
  render() {
    ...
    const clearIcon =
      (allowClear && !disabled && value.length > 0) || inputValue ? (
        <Icon type="close-circle" theme="filled" class={`${prefixCls}-picker-clear`} onClick={this.clearSelection} key="clear-icon" />
      ) : null
    ...
    const inputIcon = (suffixIcon &&
      (isValidElement(suffixIcon) ? (
        cloneElement(suffixIcon, { class: { [`${prefixCls}-picker-arrow`]: true } })
      ) : (
