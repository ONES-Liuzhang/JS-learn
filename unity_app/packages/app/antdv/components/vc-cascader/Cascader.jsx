...
import { hasProp, getEvents, getSlot, getComponentFromProp } from '../_util/props-util'
...
export default {
  ...
  methods: {
    ...
    getCurrentLevelOptions() {
      ...
      const result = arrayTreeFilter(options, (o, level) => o[this.getFieldName('value')] === sActiveValue[level], {childrenKeyName: this.getFieldName('children')})
    ...
    getActiveOptions(activeValue) {
      return arrayTreeFilter(this.options || [], (o, level) => o[this.getFieldName('value')] === activeValue[level], {childrenKeyName: this.getFieldName('children')})
    },
    setPopupVisible(popupVisible) {
      ...
      if (popupVisible && !this.sPopupVisible) {
        this.setState({ sActiveValue: this.sValue })
      }
    ...
    handleKeyDown(e) {
      ...
      if (children) {
        const { keydown } = getEvents(children)
        if (keydown) {
          keydown(e)
          return
        }
  ...
  render() {
    const { $props, $slots, sActiveValue, handleMenuSelect, sPopupVisible, handlePopupVisibleChange, handleKeyDown, $listeners } = this
    const { prefixCls, transitionName, popupClassName, options = [], disabled, builtinPlacements, popupPlacement, ...restProps } = $props
      ...
      <Trigger {...triggerProps}>
        {children &&
          cloneElement(children, {
            ...
            attrs: { tabIndex: disabled ? undefined : 0 }
          })}
