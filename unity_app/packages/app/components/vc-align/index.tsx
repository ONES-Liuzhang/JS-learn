import { Component, Prop, Vue } from 'vue-property-decorator'
import { alignElement, alignPoint } from 'dom-align'
import addEventListener from '@/app/libs/Dom/add-event-listener'
import { isWindow, buffer, isSamePoint, isSimilarValue, restoreFocus } from './util'
import { cloneElement } from '@/app/libs/vnode'
import clonedeep from 'lodash/cloneDeep'
import { getSlot } from '@/app/libs/props-util'



const getElement = (func: any) => {
  if (typeof func !== 'function' || !func) return null
  return func()
}

const getPoint = (point: any) => {
  if (typeof point !== 'object' || !point) return null
  return point
}
@Component
export default class Align extends Vue {
  @Prop() public childrenProps!: object
  @Prop({ required: true }) public align!: object
  @Prop({ default: () => window }) public target!: object | Function
  @Prop({ default: 50 }) public monitorBufferTime!: number
  @Prop({ default: false }) public monitorWindowResize!: boolean
  @Prop({ default: false }) public disabled!: boolean

  private created() {
    this.aligned = false
  }
  private mounted() {
    this.$nextTick(() => {
      this.prevProps = { ...this.$props }
      const props = this.$props
      // if parent ref not attached .... use document.getElementById
      // eslint-disable-next-line no-unused-expressions
      !this.aligned && this.forceAlign()
      if (!props.disabled && props.monitorWindowResize) {
        this.startMonitorWindowResize()
      }
    })
  }
  private updated() {
    this.$nextTick(() => {
      const { prevProps } = this
      const props = this.$props
      let reAlign = false
      if (!props.disabled) {
        const source = this.$el
        const sourceRect = source ? source.getBoundingClientRect() : null

        if (prevProps.disabled) {
          reAlign = true
        } else {
          const lastElement = getElement(prevProps.target)
          const currentElement = getElement(props.target)
          const lastPoint = getPoint(prevProps.target)
          const currentPoint = getPoint(props.target)
          if (isWindow(lastElement) && isWindow(currentElement)) {
            // Skip if is window
            reAlign = false
          } else if (
            lastElement !== currentElement || // Element change
            (lastElement && !currentElement && currentPoint) || // Change from element to point
            (lastPoint && currentPoint && currentElement) || // Change from point to element
            (currentPoint && !isSamePoint(lastPoint, currentPoint))
          ) {
            reAlign = true
          }

          // If source element size changed
          const preRect = this.sourceRect || {}
          if (!reAlign && source && (!isSimilarValue(preRect.width, (sourceRect as any).width) || !isSimilarValue(preRect.height, (sourceRect as any).height))) {
            reAlign = true
          }
        }
        this.sourceRect = sourceRect
      }

      if (reAlign) {
        this.forceAlign()
      }

      if (props.monitorWindowResize && !props.disabled) {
        this.startMonitorWindowResize()
      } else {
        this.stopMonitorWindowResize()
      }
      this.prevProps = { ...this.$props, align: clonedeep(this.$props.align) }
    })
  }
  private beforeDestroy() {
    this.stopMonitorWindowResize()
  }

  private startMonitorWindowResize() {
    if (!this.resizeHandler) {
      this.bufferMonitor = buffer(this.forceAlign, this.$props.monitorBufferTime)
      this.resizeHandler = addEventListener(window, 'resize', this.bufferMonitor)
    }
  }

  private stopMonitorWindowResize() {
    if (this.resizeHandler) {
      this.bufferMonitor.clear()
      this.resizeHandler.remove()
      this.resizeHandler = null
    }
  }

  private forceAlign() {
    const { disabled, target, align } = this.$props
    if (!disabled && target) {
      const source = this.$el

      let result
      const element = getElement(target)
      const point = getPoint(target)

      // IE lose focus after element realign
      // We should record activeElement and restore later
      const { activeElement } = document

      if (element) {
        result = alignElement(source, element, align)
      } else if (point) {
        result = alignPoint(source, point, align)
      }
      restoreFocus(activeElement, source)
      this.aligned = true
      // eslint-disable-next-line no-unused-expressions
      this.$listeners.align && (this.$listeners.align as any)(source, result)
    }
  }


  private render() {
    const { childrenProps } = this.$props
    // eslint-disable-next-line prefer-destructuring
    const child = getSlot(this)[0]
    if (child && childrenProps) {
      return cloneElement(child, { props: childrenProps })
    }
    return child
  }
}
